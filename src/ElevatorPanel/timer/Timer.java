...
public class Timer extends AppThread {
    ...
    //------------------------------------------------------------
    // Timer
    public Timer(String id, ElevatorPanel elevatorPanel) {
        super(id, elevatorPanel);
        ticker = new Ticker(getMBox());
        timerMBox = getMBox();
        timerList = new ArrayList<ActiveTimer>();
        ticks = Integer.parseInt(elevatorPanel.getProperty("Timer.MSecPerTick"));
        simulationSpeed = Integer.parseInt(elevatorPanel.getProperty("Timer.SimulationSpeed"));
        log.info(id + " simulationSpeed: " + simulationSpeed);
    } // Timer
    ...
    private void chkTimeout() {
        ...
        for (ActiveTimer timer : timeoutTimers) {
            ...
            Msg.Type returnType = timer.getReturnType();
            if(returnType != null ){
                mbox.send(new Msg("Timer", null, returnType, "[" + String.format("%05d", timerID) + "]: Time's up!"));
            } else {
    ...
    private static class ActiveTimer {
        ...
        // ActiveTimer
        public ActiveTimer(int tid, long wakeupTime, String caller, MBox mbox, Msg.Type returnType) {
            this.timerID = tid;
            this.wakeupTime = wakeupTime;
            this.caller = caller;
            this.mbox = mbox;
        ...
        public Msg.Type getReturnType() { return this.returnType; }
    ...
    public static int setTimerWithReturnType(String id, MBox mbox, long sleepTime, Msg.Type returnType) {
        int timerId = (new Random()).nextInt(90000) + 10000;
        timerMBox.send(new Msg(id, mbox, Msg.Type.SetTimer, "set timer, " + sleepTime + ", " + timerId, returnType));
        return timerId;
    } // setTimer
    ...
    public static int setSimulationTimer(String id, MBox mbox, double simulationSleepTimeInSeconds) {
        double sleepTime = simulationSleepTimeInSeconds * simulationSpeed;
        return setTimer(id, mbox,  (long) sleepTime);
    } // setSimulationTimer


    public static int setSimulationTimerWithReturnType(String id, MBox mbox, double simulationSleepTimeInSeconds, Msg.Type returnType) {
        double sleepTime = simulationSleepTimeInSeconds * simulationSpeed;
        return setTimerWithReturnType(id, mbox,  (long) sleepTime, returnType);
    }
    ...
    private void set(Msg msg) {
        ...
        Msg.Type returnType = msg.getReturnType();

        // add this new timer to timer list
        if(returnType != null){
            timerList.add(new ActiveTimer(timerID, wakeupTime, caller, mbox, returnType));
        } else {
            timerList.add(new ActiveTimer(timerID, wakeupTime, caller, mbox));
        }

        log.finest(id + ": " + caller + " setting timer: " + "[" + sleepTime + "], [" + timerID + "]" + returnType);
    } // set
